...
int main (int argc, char *argv[]){
	...
	gtk_window_set_title (GTK_WINDOW(window), "대화상자");
	gtk_window_set_position (GTK_WINDOW(window),GTK_WIN_POS_CENTER);
	gtk_window_set_default_size (GTK_WINDOW(window), 200, 80);
	g_signal_connect ( window, "destroy", G_CALLBACK(quit), NULL);
	dialog = gtk_dialog_new_with_buttons ("긴급!",GTK_WINDOW (window),GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_YES, GTK_RESPONSE_YES, GTK_STOCK_NO, GTK_RESPONSE_NO, NULL);
